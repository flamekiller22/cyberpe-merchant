...
export default function Home({ email, balance, tranhist }) {
  return (
    <div>
...
export async function getServerSideProps(context) {
  ...
  const balance = await fetch(`http://127.0.0.1:5001/check-store-balance?email=${email}`).then(res => res.json())
  const tranhist = await fetch(`http://127.0.0.1:5001/store-transaction-history?email=${email}`).then(res => res.json())
